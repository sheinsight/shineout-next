...
当用户需要评估项目升级 Shineout 版本的风险时，按以下流程生成报告：

1. **确定版本跨度**
   - 找到项目当前使用的 shineout 版本（查看 package.json）
   - 确定目标升级版本
   - 列出中间跨越的所有正式版本

2. **识别受影响组件**
   - 扫描 `packages/shineout/src/[component]/__diff__/` 目录
   - 找出版本跨度内有 diff 报告的组件

3. **分析组件修复问题**
   - 读取每个组件在版本跨度内的所有 diff 报告
   - 提取修复的问题、影响场景和代码模式
...
|---------|----------|---------------|----------|
| `[相对路径:行号]` | [问题描述] | [版本号]：[具体问题] | **高** |

### 中风险文件

| 文件路径 | 问题场景 | 相关版本及问题 | 风险等级 |
|---------|----------|---------------|----------|
| `[相对路径:行号]` | [问题描述] | [版本号]：[具体问题] | **中** |

### 低风险文件

| 文件路径 | 问题场景 | 相关版本及问题 | 风险等级 |
|---------|----------|---------------|----------|
| `[相对路径:行号]` | [问题描述] | [版本号]：[具体问题] | **低** |
...
1. **Form 组件常见风险点**
   - 受控与非受控模式冲突（defaultValue vs value）
   - Form.Flow 配置问题
   - formRef.set 对象更新
   - onChange 参数直接修改
   - 条件渲染的表单字段
...
3. **其他组件风险识别模式**
   - 查看 changelog 中的 BugFix 内容
   - 关注 beta 版本的修复说明
   - 识别影响范围和使用场景

### 报告文件命名规范

- 文件名：`shineout-upgrade-risk-assessment.md`
- 位置：项目根目录或指定目录
- 编码：UTF-8

### 注意事项

1. **精确定位**：必须提供具体的文件路径和行号
2. **版本追溯**：标注问题出现的具体 beta 版本范围
3. **场景描述**：清晰描述问题发生的使用场景
4. **统计准确**：提供受影响文件的准确数量统计
