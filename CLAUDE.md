...
### diff 报告注意事项
diff 报告需要根据 changelog 概览中描述的版本，并结合该版本的 commit 以及前后相关的 commit 记录来生成，具体内容包括：
- 问题描述。内容同概览，需要根据具体 commit 信息详细加以描述变更内容和变更情况
- 代码变文件。本次变更影响到的所有文件的相对路径
- 变更代码行。本次 changelog 涉及到的所有代码变化的所有地方都要罗列出来
- 变更前后逻辑差异。包含变更前、变更后的逻辑差异，需说明变更处代码对当前组件上下文乃至整个组件的运作逻辑的影响，例如将 if(!str) 修改为 if(str!=='') 收窄了判断单独过滤了''，但依然能接受部分假值，需要分析该调整对组件内部以及对组件外部造成的影响，以便后续进行变更风险分析
- 风险使用场景。包含代码执行风险和交互体验差异，根据前后变更带来的逻辑差异和逻辑影响范围，综合分析本次变更对代码执行和交互体验有何影响，对使用层面是否造成差异或 breakchange，需要罗列出可能影响的场景和风险