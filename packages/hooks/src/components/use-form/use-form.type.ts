...
export interface FormLabelConfig {
   ...
   * @description 统一设置表单项标签的宽度。可以是具体像素值（如 '140px'）或百分比（如 '30%'）。当标签顶部对齐时此配置无效。用于保持表单整齐的视觉效果，确保所有表单项的输入区域左对齐
   * @default 140px
   */
   ...
   * @cn 默认为空，跟随主题样式。
   * @description 标签的对齐方式。'left' 左对齐，'right' 右对齐，'top' 顶部对齐。不同的对齐方式适用于不同的表单布局需求：'right' 适合传统表单布局，'top' 适合移动端或紧凑布局，'left' 适合特殊设计需求
   */
  labelAlign?: 'left' | 'right' | 'top';
  /**
   * @en the default value is top.
   * @cn 默认顶部对齐
   * @description 标签的垂直对齐方式。当标签和输入框在同一行时（labelAlign 为 'left' 或 'right'），控制标签相对于输入框的垂直位置。'top' 与输入框顶部对齐，'middle' 居中对齐，'bottom' 与输入框底部对齐
   * @default 'top'
   */
   ...
   * @cn 单行错误提示不撑开页面高度
   * @description 保持错误提示高度固定，避免错误信息出现/消失时页面抖动。开启后会预留错误信息的显示空间，确保表单高度稳定。适用于对布局稳定性要求较高的场景，如表单项较多的长表单。注意：当 keepErrorBelow 为 true 时，此配置会失效
   * @default false
   */
   ...
   * @cn 是否保持错误信息在提示信息下方
   * @description 控制错误信息的显示位置。开启后错误信息始终显示在表单项的 tip 属性下方，保持布局的一致性。关闭时错误信息可能会覆盖 tip 信息。适用于同时需要显示提示信息和错误信息的场景。注意：开启后会使 keepErrorHeight 失效
   * @default false
   * @version 3.7.0
   */
   ...
   * @cn 是否水平布局
   * @description 将表单项横向排列，适用于简单的单行表单或筛选表单。开启后表单项会在同一行显示，自动换行。通常用于搜索条件、筛选器等需要节省垂直空间的场景
   * @default false
   */
  inline?: boolean;
   ...
   * @cn 标签后是否追加显示一个冒号，优先级小于 Form.Item 的 colon 设置
...
export interface FormCommonConfig extends FormLabelConfig {
   ...
   * @description 全局控制表单的禁用状态。适用于查看详情、提交中等需要临时禁用整个表单的场景。表单内所有受控组件（如 Input、Select、DatePicker 等）都会继承此属性
   * @default false
   */
  disabled?: boolean;
  /**
   ...
   * @cn 表单元素的尺寸
   * @description 统一设置表单内所有组件的尺寸。支持 'small'、'default'、'large' 三种规格，影响输入框高度、字体大小等。表单内的受控组件会继承此属性，也可以在具体组件上单独覆盖
   * @default 'default'
   */
   ...
   * @cn 设置为 true 时，表单内所有组件的 reserveAble 都为 true : 卸载后不自动删除数据
   * @description 控制表单项卸载时是否保留数据。适用于标签页切换、条件渲染等场景，避免用户填写的数据丢失。开启后即使表单项被隐藏或卸载，其数据仍会保留在表单中。可在 Form.Field 组件或支持的表单组件（如 Input、Select、DatePicker 等）上单独设置覆盖此全局配置
   * @default false
   * @version 3.5.0
   */
  reserveAble?: boolean;
...
export interface BaseFormProps<T> extends Omit<FormCommonConfig, 'formName'> {
  ...
  defaultValue?: T;
  /**
   * @en Whether to validate automatically after setting value
   * @cn 设置 value 后是否自动校验
   ...
   * @description 提升用户体验的重要配置。当表单较长时，自动滚动到错误位置可以帮助用户快速定位问题。设置为 true 时使用默认滚动，设置为数字时可以调整滚动位置的偏移量，避免被固定头部遮挡
   * @default false
   */
  ...
  /**
   * @en Whether to remove fields with undefined values
  ...
  /**
   * @en Throttle time for submit (ms) to prevent duplicate submission
