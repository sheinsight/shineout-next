...
1. **虚拟列表项高度计算变化**：
   - 变更描述：当设置 `lineHeight="auto"` 时，列表项从固定高度变为动态高度
   - 风险场景：依赖固定高度进行样式计算或布局的场景
   - 风险示例：
   ```css
   /* 风险样式：假设列表项高度固定 */
   .so-select-option {
   ...
   /* 依赖固定高度的定位 */
   .custom-tooltip {
     top: calc(32px * var(--index));
   }
   ```
   - 规避方案：使用相对定位或移除固定高度依赖

### 行为逻辑变更风险

1. **滚动事件监听冲突**：
   - 变更描述：新增滚动加载功能会监听滚动事件
   - 风险场景：业务代码已经监听了下拉列表的滚动事件
   - 风险示例：
   ```tsx
...
2. **触发方式变更影响**：
   ...
   ```
   - 规避方案：使用 onOpenChange 回调替代点击事件监听

3. **空数据渲染逻辑变化**：
   - 变更描述：`emptyText={false}` 时改变了渲染逻辑
   - 风险场景：依赖原有空数据提示行为的场景
   - 风险示例：
   ```tsx
   // 风险代码：依赖默认空提示的存在
...
- **无相关风险**：本次变更主要是功能增强，未涉及样式类名或结构的破坏性变更
