...
describe('TreeSelect[Base]', () => {
  ...
  test('should check on full data when checkOnFiltered is false', async () => {
    const onChange = jest.fn();
    const testData = [
      {
        id: '1',
    ...
    if (treeRoot) {
      ...
      if (firstNode) {
        ...
        if (checkbox) {
          ...
          // 关键验证：checkOnFiltered=false 时，即使筛选了 '1-1'
          // 选中父节点应该选中完整数据集中的所有子节点
          // 包括：1-1-1, 1-1-2, 1-2（因为父节点 1 下的所有叶子节点都应该被选中）
          expect(selectedValues).toBeDefined();
          expect(Array.isArray(selectedValues)).toBe(true);

          // 应该包含 node 1 下的所有叶子节点
          expect(selectedValues).toContain('1-1-1');
          expect(selectedValues).toContain('1-1-2');
          // 这是关键：checkOnFiltered=false 时应该包含 1-2（虽然它不匹配筛选条件）
          expect(selectedValues).toContain('1-2');

          // 验证总数应该是3个（1-1-1, 1-1-2, 1-2）
