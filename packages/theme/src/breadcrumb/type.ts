// 此文件由脚本自动生成，请勿直接修改。
// This file was generated automatically by a script. Please do not modify it directly.

export interface BreadcrumbTokens {
  /**
   * @type {string}
   * @token Font-14
   * @description 面包屑字体字号
   */
  breadcrumbFontSize: string;
  /**
   * @type {string}
   * @token Neutral-text-5
   * @description 面包屑字体颜色
   */
  breadcrumbFontColor: string;
  /**
   * @type {string}
   * @token Weight-regular
   * @description 面包屑字体字重
   */
  breadcrumbFontWeight: string;
  /**
   * @type {string}
   * @token Neutral-text-3
   * @description 面包屑链接色颜色
   */
  breadcrumbLinkColor: string;
  /**
   * @type {string}
   * @token Brand-5
   * @description 面包屑链接色鼠标移入时的颜色
   */
  breadcrumbLinkHoverColor: string;
  /**
   * @type {string}
   * @token Brand-6
   * @description 面包屑默认尺寸链接色颜色
   */
  breadcrumbDefaultLinkColor: string;
  /**
   * @type {string}
   * @token Brand-5
   * @description 面包屑默认尺寸链接色鼠标移入时的颜色
   */
  breadcrumbDefaultLinkHoverColor: string;
  /**
   * @type {string}
   * @token Brand-7
   * @description 面包屑默认尺寸链接色鼠标点击时的颜色
   */
  breadcrumbDefaultLinkActiveColor: string;
  /**
   * @type {string}
   * @token Spacing-4
   * @description 面包屑下拉列表的内边距（垂直方向）
   */
  breadcrumbListPaddingY: string;
  /**
   * @type {string}
   * @token Radius-default
   * @description 面包屑下拉列表的选项边框圆角
   */
  breadcrumbListItemBorderRadius: string;
  /**
   * @type {string}
   * @token Spacing-4
   * @description 面包屑下拉列表的选项外部容器内边距（水平方向）
   */
  breadcrumbListItemWrapperPaddingX: string;
  /**
   * @type {string}
   * @token Spacing-8
   * @description 面包屑下拉列表的选项内边距（水平方向）
   */
  breadcrumbListItemPaddingX: string;
  /**
   * @type {string}
   * @token Spacing-5
   * @description 面包屑下拉列表的选项内边距（垂直方向）
   */
  breadcrumbListItemPaddingY: string;
  /**
   * @type {string}
   * @token Neutral-fill-2
   * @description 面包屑下拉列表的选项鼠标移入时的背景颜色
   */
  breadcrumbListItemHoverBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-3
   * @description 面包屑下拉列表的选项鼠标移入时的字体颜色
   */
  breadcrumbListItemHoverFontColor: string;
  /**
   * @type {string}
   * @token Weight-regular
   * @description 面包屑上级字体字重
   */
  breadcrumbPrevFontWeight: string;
  /**
   * @type {string}
   * @token Spacing-8
   * @description 面包屑分隔符外边距（水平方向）
   */
  breadcrumbSeparatorMarginX: string;
  /**
   * @type {string}
   * @token Neutral-text-4
   * @description 面包屑图标背景颜色
   */
  breadcrumbIconBackgroundColor: string;
}
