// 此文件由脚本自动生成，请勿直接修改。
// This file was generated automatically by a script. Please do not modify it directly.

export interface MenuTokens {
  /**
   * @type {string}
   * @token Font-14
   * @description 菜单字体字号
   */
  menuFontSize: string;
  /**
   * @type {string}
   * @token Neutral-text-5
   * @description 菜单字体颜色
   */
  menuFontColor: string;
  /**
   * @type {string}
   * @token Neutral-text-4
   * @description 菜单图标颜色
   */
  menuIconColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-1
   * @description 菜单选项背景颜色
   */
  menuItemBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-5
   * @description 菜单选项字体颜色
   */
  menuItemFontColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-2
   * @description 菜单选项鼠标移入时的背景颜色
   */
  menuItemHoverBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-5
   * @description 菜单选项鼠标移入时的字体颜色
   */
  menuItemHoverFontColor: string;
  /**
   * @type {string}
   * @token Brand-1
   * @description 菜单选项鼠标点击时的背景颜色
   */
  menuItemActiveBackgroundColor: string;
  /**
   * @type {string}
   * @token Brand-2
   * @description 菜单选项鼠标点击时的鼠标移入时的背景颜色
   */
  menuItemActiveHoverBackgroundColor: string;
  /**
   * @type {string}
   * @token Brand-6
   * @description 菜单选项鼠标点击时的字体颜色
   */
  menuItemActiveFontColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-1
   * @description 菜单选项禁用时的背景颜色
   */
  menuItemDisabledBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-2
   * @description 菜单选项禁用时的字体颜色
   */
  menuItemDisabledFontColor: string;
  /**
   * @type {string}
   * @token 16px
   * @description 菜单标题模式内边距（水平方向）
   */
  menuTitlePaddingX: string;
  /**
   * @type {string}
   * @token 9px
   * @description 菜单标题模式内边距（垂直方向）
   */
  menuTitlePaddingY: string;
  /**
   * @type {string}
   * @token 46px
   * @description 菜单展开icon的宽度
   */
  menuExpandWidth: string;
  /**
   * @type {string}
   * @token 14px
   * @description 菜单展开icon的字号
   */
  menuExpandSize: string;
  /**
   * @type {string}
   * @token Neutral-fill-3
   * @description 菜单展开icon的鼠标移入时的背景颜色
   */
  menuExpandHoverBackgroundColor: string;
  /**
   * @type {string}
   * @token 0 2px 8px rgba(0, 0, 0, 0.15)
   * @description 菜单子菜单阴影
   */
  menuChildrenBoxShadow: string;
  /**
   * @type {string}
   * @token Neutral-text-1
   * @description 菜单深色模式字体颜色
   */
  menuDarkFontColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-10
   * @description 菜单深色模式选项背景颜色
   */
  menuDarkItemBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-1
   * @description 菜单深色模式选项字体颜色
   */
  menuDarkItemFontColor: string;
  /**
   * @type {string}
   * @token Brand-9
   * @description 菜单深色模式选项鼠标移入时的背景颜色
   */
  menuDarkItemHoverBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-1
   * @description 菜单深色模式选项鼠标移入时的字体颜色
   */
  menuDarkItemHoverFontColor: string;
  /**
   * @type {string}
   * @token Brand-6
   * @description 菜单深色模式选项鼠标点击时的背景颜色
   */
  menuDarkItemActiveBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-1
   * @description 菜单深色模式选项鼠标点击时的字体颜色
   */
  menuDarkItemActiveFontColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-9
   * @description 菜单深色模式选项打开状态的背景颜色
   */
  menuDarkItemOpenBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-1
   * @description 菜单深色模式选项打开状态的字体颜色
   */
  menuDarkItemOpenFontColor: string;
  /**
   * @type {string}
   * @token Neutral-fill-10
   * @description 菜单深色模式选项禁用时的背景颜色
   */
  menuDarkItemDisabledBackgroundColor: string;
  /**
   * @type {string}
   * @token Neutral-text-4
   * @description 菜单深色模式选项禁用时的字体颜色
   */
  menuDarkItemDisabledFontColor: string;
}
