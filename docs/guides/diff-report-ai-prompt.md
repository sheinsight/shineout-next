...
1. 严格按照以下章节顺序生成，不要使用编号：
   ...
   - 变更前后逻辑差异
   - 逻辑影响范围
   - 风险场景分析（包含"DOM 结构变更风险"、"行为逻辑变更风险"和"样式变更风险"子章节）
...
3. 风险场景分析必须包含：
   - DOM 结构变更风险：分析 DOM 结构变化，提供可能依赖的选择器示例
   - 行为逻辑变更风险：识别行为变化可能导致的问题场景
   - 样式变更风险：检查样式类名变化对自定义样式的影响
   - 每个风险都要有具体的风险代码示例和规避方案
   - 如无相关风险，明确标注"无相关风险"

4. 升级注意事项的"行为变化说明"必须包含：
   - 影响场景
   - 具体表现
   - 受影响代码示例（带有"之前/现在"注释）
   - 是否需要调整
...
1. 包含且仅包含 8 个指定章节
2. 没有使用编号的章节标题
3. 风险场景分析包含三个子章节（DOM、行为逻辑、样式）
4. 每个风险都有具体的风险示例和规避方案
5. 升级注意事项包含完整的行为变化说明
6. 代码示例有"之前/现在"的对比
7. 明确告知用户是否需要调整代码
